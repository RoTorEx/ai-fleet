...
AI Fleet is a tiny native macOS menu-bar application built with SwiftUI. It shows the current status and remaining limit for two lanes — Kimi and Codex — and refreshes once per minute.
...
2. `StatusService.start()` begins polling Kimi and Codex every 60 seconds.
...
5. Each provider returns a `ProviderStatus` with `ok`, `limited`, `offline`, or `noKey` state.
